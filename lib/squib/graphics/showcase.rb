module Squib
  class Deck
    ...
    def render_showcase(range, trim, trim_radius, dir, file_to_save)
      ...
      out_cc.set_source_color(:black)
      out_cc.paint

      cards = range.collect { |i| @cards[i] }

      cards.each_with_index do |card, i|
        surface = perspective_reflect(card.cairo_surface, angle, scale)
        out_cc.set_source(surface, 25 + i * (surface.width + 25) ,75)
        out_cc.paint
      end
    ...
    def perspective_reflect(surface, angle, scale)
      tmp_cc = Cairo::Context.new(Cairo::ImageSurface.new(surface.width * (scale+angle/2), surface.height * (2 + scale)))
      tmp_cc.set_source_color(:gray)
      tmp_cc.paint
      ...
      # Gradient mask
      gradient = Cairo::LinearPattern.new(0.0, 1000, 0.0, 1000)
      gradient.add_color_stop_rgba(0, 0, 0, 0, 0.0)
      gradient.add_color_stop_rgba(1.0, 0, 0, 0, 0.5)
